...
const clickme = document.getElementById("clickme") as HTMLButtonElement;
...
const randomPicture = () => {
    //random nummer generieren? loop?
    //Math.random / for loop mit length?
    ...
    const img = document.createElement("img"); // create list element
    img.src = data[randomNumber].download_url; // in list element 'upload' whatever user wrote
    // console.log(data[randomNumber]);

    output?.appendChild(img);
    //danach create element
    //createElement
...
//randomPicture(); brauch ich doch nicht

clickme.addEventListener("click", randomPicture);
